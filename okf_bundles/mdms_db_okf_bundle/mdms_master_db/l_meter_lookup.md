...
* Joined with [l_consumer_table](mdms_master_db/l_consumer_lookup.md) (alias: `lcl`) ON `lml.MeterLookup_TblRefID` = `lcl.MeterLookup_TblRefId`

*(Note: Depending on your other master tables, you will likely add joins here for `NetworkLookup_TblRefID`, `DeviceManufacturer_TblRefID`, `TimeOfDay_TblRefID`, etc. Use the same explicit `lml.column = foreign.column` format when you do!)*